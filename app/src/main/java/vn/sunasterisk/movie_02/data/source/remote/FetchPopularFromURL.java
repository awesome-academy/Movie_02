...
public class FetchPopularFromURL extends AsyncTask<String, Void, List<Popular>> {
    ...
    private List<Popular> getPopularFromStringData(String data) throws JSONException {
        ...
        JSONArray jsonArray = object.getJSONArray(PopularEntity.POPULAR);
        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject jsonObject = (JSONObject) jsonArray.get(i);
            String popularity = jsonObject.getString(PopularEntity.POPULARITY);
            String video = jsonObject.getString(PopularEntity.VIDEO);
            String posterPath = jsonObject.getString(PopularEntity.POSTERPATH);
            String id = jsonObject.getString(PopularEntity.ID);
            String backdropPath = jsonObject.getString(PopularEntity.BACKDROPPATH);
            String title = jsonObject.getString(PopularEntity.TITLE);
            String voteAverage = jsonObject.getString(PopularEntity.VOTEAVERAGE);
            String overView = jsonObject.getString(PopularEntity.OVERVIEW);
            String releaseDate = jsonObject.getString(PopularEntity.RELEASEDATE);

            Popular popular = new Popular(popularity, video, posterPath, id, backdropPath,
